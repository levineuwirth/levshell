...
\subsubsection{levshell-ctl: CLI Control Interface}
A standalone binary (\texttt{levshell-ctl}) that connects to the daemon's Unix socket, sends a single command, prints the response, and exits. This enables external control of the shell from any context: Sway keybinds, shell scripts, cron jobs, or other tools. The command format follows a \texttt{target / action / params} convention:

\begin{lstlisting}[language={}]
levshell-ctl widget ssh-dashboard reconnect gpu-cluster-3
levshell-ctl palette query "zotero attention mechanisms"
levshell-ctl density compact
levshell-ctl profile activate writing
levshell-ctl project status llm-alignment
levshell-ctl anki due-count
levshell-ctl notify "Build finished" --urgency normal
\end{lstlisting}

This binary reuses the existing IPC message types (\texttt{ShellMessage} enum) and codec from \texttt{levshell-ipc}. It is a thin client: connect, serialize one message, read one response, print, exit. The daemon handles \texttt{levshell-ctl} connections alongside the primary QML shell connection by accepting multiple clients on the Unix socket. Integration with Sway keybinds is direct:

\begin{lstlisting}[language={}]
# In sway config
bindsym $mod+space exec levshell-ctl palette toggle
bindsym $mod+shift+n exec levshell-ctl density toggle
bindsym $mod+p exec levshell-ctl profile cycle
\end{lstlisting}
...
A Unix domain socket carrying NDJSON (newline-delimited JSON) messages. Each message is a single line of compact JSON terminated by \texttt{\textbackslash n}. JSON's grammar forbids unescaped newlines inside strings, making \texttt{\textbackslash n} a safe frame delimiter. This format was chosen over length-prefixed binary framing because QuickShell's \texttt{Io.Socket} API provides a \texttt{SplitParser} that splits on string delimiters but has no byte-level framing support. A per-line size cap of 16\,MB guards against malformed frames that could stall the parser.

Messages are never pretty-printed on the wire (compact single-line JSON only). The QML side parses each line with \texttt{JSON.parse()}; the Rust side serializes with \texttt{serde\_json::to\_string()} and appends \texttt{\textbackslash n}.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=4pt]
  ...
  \item \textbf{Module relevance scores.} Each module declares relevance rules as predicates. Rules specify conditions under which a widget should be visible, compact, badge, expanded, or hidden. Conditions reference input signals and module-internal state.
...
Widget prominence is resolved by a strict, deterministic cascade. Given the same input signals, the algorithm always produces the same output. The prominence levels form a total order: \texttt{hidden} $<$ \texttt{badge} $<$ \texttt{icon-only} $<$ \texttt{compact} $<$ \texttt{visible} $<$ \texttt{expanded}.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=4pt]
  ...
  \item \textbf{Step 4 --- Spatial constraint enforcement.} The bar has a finite pixel budget. Walk the widget list in \emph{global priority order} (a user-configurable ordered list that ships with sensible defaults). Place each widget at its resolved prominence. If placing a widget at its current prominence would overflow the available space, demote it one level (e.g., \texttt{expanded} $\to$ \texttt{compact} $\to$ \texttt{icon-only} $\to$ \texttt{badge} $\to$ \texttt{hidden}). Highest-priority widgets receive first claim on bar real estate. This is a greedy $O(n)$ algorithm.
...
levshell/
  ...
  crates/
    ...
    levshell-ctl/             # CLI binary: one-shot commands to daemon
                              # via Unix socket (connect, send, print, exit)
...
WidgetVisibility {
    widget_id: String,
    visible: bool,
    prominence: Prominence,       // Hidden | Badge | IconOnly | Compact
                                  //   | Visible | Expanded
}
...
\begin{designbox}[Phase 0 --- Complete]
Phase~0 shipped with 35 tests across the workspace (8 data + 10 core + 11 ipc + 5 indicator + 1 daemon end-to-end), zero clippy warnings. Delivered: Cargo workspace scaffold (8 crates), unified data model with SQLite schema and migrations, typed event bus with per-subscriber filtering, NDJSON IPC over Unix socket with codec abstraction, Sway workspace module, daemon binary with \texttt{ModuleFactory} pattern, and a minimal QuickShell QML bar.

\textbf{Phase 0 deviations from original spec:} IPC framing was changed from 4-byte length-prefixed binary to NDJSON (newline-delimited JSON) because QuickShell 0.2.1's \texttt{Io.Socket} only supports \texttt{SplitParser} with string delimiters, not byte-level framing. The 16\,MB per-line cap is retained. The \texttt{DataStoreUpdated} event uses \texttt{String} for \texttt{entity\_type} (not \texttt{levshell\_data::EntityType}) to keep \texttt{levshell-core} as a dependency-free leaf crate. Reference CRUD is minimal (insert + get only); other entity types have model structs but no repo methods yet. The data store uses a single \texttt{Mutex<Connection>} rather than a pool---adequate for Phase~0 write volumes.
\end{designbox}

\subsection{Phase 1: Core Shell}

With the foundation proven, build the core shell features: the context engine, essential bar widgets, the CLI control interface, and the visual language. Phase~1 turns the infrastructure into a usable daily-driver shell.

\subsubsection{Step 1.1: levshell-ctl CLI}

Create the \texttt{levshell-ctl} binary crate. This is a thin CLI client that connects to the daemon's Unix socket, sends a single command (serialized as a \texttt{ShellMessage}), reads one response, prints it, and exits. It reuses \texttt{levshell-ipc} message types and the NDJSON codec.

\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item Add \texttt{crates/levshell-ctl/} to the workspace. Dependencies: \texttt{levshell-ipc}, \texttt{tokio} (minimal features), \texttt{clap} for argument parsing, \texttt{anyhow}.
  \item Implement the \texttt{target / action / params} CLI grammar:
  \begin{lstlisting}[language={}]
levshell-ctl widget <widget-id> <action> [params...]
levshell-ctl palette <toggle|query> [query-string]
levshell-ctl density <full|compact|hidden|toggle>
levshell-ctl profile <activate|cycle> [profile-name]
levshell-ctl status                # Daemon health summary
  \end{lstlisting}
  \item Modify \texttt{levshell-daemon} to accept multiple concurrent connections on the Unix socket: the primary QML shell connection (persistent, streaming) and zero or more \texttt{levshell-ctl} connections (ephemeral, one-shot). The daemon distinguishes them by behavior---a \texttt{ctl} connection sends one message and the daemon responds then closes, while the shell connection remains open.
  \item Add a \texttt{CtlResponse} message type to the daemon$\to$client direction for one-shot command results (distinct from the streaming \texttt{DaemonMessage} types used for the QML shell).
  \item Write integration tests: \texttt{levshell-ctl density compact} while the daemon is running, verify the density change event is emitted on the bus.
\end{enumerate}

\begin{notebox}[Sway Keybind Integration]
Once \texttt{levshell-ctl} is functional, Sway keybinds can control Levshell directly:
\begin{lstlisting}[language={}]
# In ~/.config/sway/config
bindsym $mod+space exec levshell-ctl palette toggle
bindsym $mod+shift+n exec levshell-ctl density toggle
bindsym $mod+p exec levshell-ctl profile cycle
bindsym $mod+shift+s exec levshell-ctl widget ssh-dashboard reconnect
\end{lstlisting}
This gives Levshell the same external scriptability as DankMaterialShell's \texttt{dms ipc call} pattern, while keeping the persistent streaming socket for high-frequency QML updates.
\end{notebox}

\subsubsection{Step 1.2: Context Engine}

In \texttt{levshell-context}, implement:

\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item The five-step conflict resolution algorithm (Section~\ref{sec:context-engine}): base layout $\to$ module relevance $\to$ active profile $\to$ spatial constraints $\to$ user override.
  \item The relevance rule predicate DSL, parsed from TOML configuration files. Rules reference context signals (workspace tags, focused app\_id, module-internal conditions) and specify a target prominence level.
  \item The hysteresis system: activation delay (default 2s), deactivation delay (default 10s), configurable per-rule.
  \item Context profile loading from \texttt{\textasciitilde/.config/levshell/profiles/*.toml}.
  \item Integration with the event bus: subscribe to \texttt{WorkspaceChanged}, \texttt{WindowFocused}, and \texttt{ProfileActivated} events; recompute widget layout on each relevant event; push \texttt{BarLayout} and \texttt{WidgetVisibility} messages over IPC.
  \item Tests: verify deterministic resolution across all five steps, hysteresis prevents flicker on oscillating signals, profile overrides take precedence over module rules, spatial constraint demotion works correctly.
\end{enumerate}

\subsubsection{Step 1.3: System Telemetry Modules}

Self-contained modules that exercise the module trait and widget rendering without needing sync adapters or the unified data model:

\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{CPU/RAM monitor.} Reads from \texttt{/proc/stat} and \texttt{/proc/meminfo}. Emits sparkline data. Implements ``quiet until important'' --- widget is compact by default, expands when CPU $>$ 60\% or RAM $>$ 80\%.
  \item \textbf{Battery module.} Reads from \texttt{/sys/class/power\_supply/}. Emits \texttt{PowerStateChanged} events. Shows estimated time remaining, power profile switcher.
  \item \textbf{Network quality.} Periodic latency probe to a configurable endpoint. Color-coded dot (green/yellow/red).
  \item Each module declares relevance rules so the context engine can test real layout resolution with multiple competing widgets.
\end{enumerate}

\subsubsection{Step 1.4: Core Bar Widgets and Visual Language}

Build the QML components that establish the shell's visual identity:

\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Clock and calendar hub.} Center clock that drops down to show a mini calendar, upcoming events placeholder (will be populated when CalDAV sync adapter lands in Phase~2), and world-clock row.
  \item \textbf{Quick-settings flyout.} Volume, brightness, night-light, Wi-Fi toggle, Bluetooth toggle, Do Not Disturb. Each tile backed by a daemon module or a \texttt{levshell-ctl} action.
  \item \textbf{Notification center.} Implement the freedesktop notification spec listener (via D-Bus) as a daemon module. QML renders grouped notifications with inline reply support for supported apps.
  \item \textbf{Morphing bar density.} Implement the three density modes (full, compact, hidden) with animated transitions, controllable via \texttt{levshell-ctl density} and keybind.
  \item \textbf{Degraded-state wrapper.} The shared QML component that applies visual treatment (dimming, warning icon, tooltip) for widgets in Stale or Error states. Every widget uses this wrapper.
\end{enumerate}

\subsubsection{Step 1.5: Command Palette}

\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item A search-driven overlay triggered by keybind (via \texttt{levshell-ctl palette toggle}).
  \item Search provider interface: each provider registers a name and a query handler. Initial providers: application launcher (reading \texttt{.desktop} files), workspace switcher, and unified data model search (notes and references via FTS5).
  \item Results are categorized, ranked, and rendered in sections. Selecting a result dispatches to the provider's action handler.
\end{enumerate}

\begin{designbox}[Phase 1 Completion Criteria]
Phase~1 is complete when: (1) \texttt{levshell-ctl} can send commands to the daemon and receive responses; (2) the context engine resolves widget layout deterministically from workspace/window signals, module relevance, and profiles; (3) at least three system telemetry modules render live data in the bar; (4) the notification center captures and displays desktop notifications; (5) the command palette launches applications and searches the data model; (6) the bar supports all three density modes with animated transitions. At this point, Levshell is a functional daily-driver shell (without research-specific features).
\end{designbox}
